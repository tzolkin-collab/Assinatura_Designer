...
const referenciaSchema = z.object({
  name: z
    .string({ required_error: "Reference name is required" })
    .trim()
    ...
    try {
      ...
      const config = await prisma.brandConfig.upsert({
        ...
        create: {
          ...
          autoResearchInterval: autoResearchInterval
            ? Number(autoResearchInterval)
            : 14,
        },
      });
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!parsed.success)
        throw createError(
          400,
          parsed.error.errors[0]?.message ?? "Corpo da requisição inválido",
        );
      const {
        name,
        analysisUrl,
      ...
      if (!isPublicHttpUrl(analysisUrl))
        throw createError(
          400,
          "URL inválida ou não permitida (apenas http(s) público)",
        );

      let validSourceType: "WEBSITE" | "INSTAGRAM" =
        sourceType === "INSTAGRAM" ? "INSTAGRAM" : "WEBSITE";
...
settingsRouter.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!ref) throw createError(404, "Reference not found");
      if (!ref.analysisUrl)
        throw createError(400, "Referência sem URL de análise configurada");
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!file.mimetype.startsWith("image/"))
        throw createError(400, "O arquivo precisa ser uma imagem.");
      ...
      res.json({
        data: updated,
        message: "Análise com imagem enviada iniciada",
      });
